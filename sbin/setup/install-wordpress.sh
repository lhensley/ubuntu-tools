...
echo "C01"
mysql -u $mysql_root_user -p$mysql_root_password -e "$query"
echo "D01"
query="FLUSH PRIVILEGES; "
echo "C02"
mysql -u $mysql_root_user -p$mysql_root_password -e "$query"
echo "D02"
query="CREATE USER 'wp_$fqdn'@'localhost' IDENTIFIED WITH caching_sha2_password BY '$mysql_wp_pwd';"
echo "C03"
mysql -u $mysql_root_user -p$mysql_root_password -e "$query"
echo "D03"
query="GRANT USAGE ON *.* TO 'wp_$fqdn'@'localhost';"
echo "C04"
mysql -u $mysql_root_user -p$mysql_root_password -e "$query"
echo "D04"
query="ALTER USER 'wp_$fqdn'@'localhost' REQUIRE NONE WITH MAX_QUERIES_PER_HOUR 0 MAX_CONNECTIONS_PER_HOUR 0 MAX_UPDATES_PER_HOUR 0 MAX_USER_CONNECTIONS 0;"
echo "C05"
mysql -u $mysql_root_user -p$mysql_root_password -e "$query"
echo "D05"
query="CREATE DATABASE IF NOT EXISTS `wp_$fqdn`;"
echo "C06"
mysql -u $mysql_root_user -p$mysql_root_password -e "$query"
echo "D06"
query="GRANT ALL PRIVILEGES ON `wp_$fqdn`.* TO 'wp_$fqdn'@'localhost';"
echo "C07"
mysql -u $mysql_root_user -p$mysql_root_password -e "$query"
echo "D07"
exit 1
